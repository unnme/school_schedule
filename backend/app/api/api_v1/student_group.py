...
async def create_sudent_group(
    ...
    request_data: StudentGroupCreateRequest
) -> StudentGroupCreateResponse:
    created_student_group = await StudentGroupManager.create_student_group(session, request_data)
    return StudentGroupCreateResponse(message="Группа учеников успешно создана", data=created_student_group)


@router.get("/", response_model=list[StudentGroupResponse])
async def get_sudent_groups(session: AsyncSessionDep, params: PaginationParams) -> Sequence[StudentGroupResponse]:
...
async def update_sutdent_group(
    ...
    return _StudentGroupUpdateResponse.model_validate(message="Группа учеников успешно обновлёна", data=udated_student_group)


@router.delete("/{student_group_id}", response_model=StudentGroupDeleteResponse)
async def delete_student_group(db: AsyncSessionDep, student_group_id: int) -> StudentGroupDeleteResponse:
    deleted_student_group = await StudentGroupManager.delete_sutdent_group(
        db, student_group_id
    )
    return {"message": "Группа учеников успешно удалена", "data": deleted_student_group}
