...
async def create_subject(session: AsyncSessionDep, request_data: SubjectCreateRequest) -> SubjectCreateResponse:
...
async def get_subjects(params: PaginationParams, session: AsyncSessionDep) -> Sequence[SubjectResponse]:
...
async def update_subject(
    ...
    return SubjectUpdateResponse(message="Предмет успешно изменен", data=updated_subject)



@router.delete("/{subject_id}", response_model=SubjectDeleteResponse)
async def delite_subject(session: AsyncSessionDep, subject_id: int) -> SubjectDeleteResponse:
